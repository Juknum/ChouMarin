...
Scheduler::Scheduler(const Scheduler& scheduler_p):m_server(scheduler_p.m_server),m_light(scheduler_p.m_light),
    m_temperature(scheduler_p.m_temperature),m_humidity(scheduler_p.m_humidity),m_sound(scheduler_p.m_sound),
    m_pression(scheduler_p.m_pression)
{
...
void Scheduler::Launch()
{
  ...
  //for the first version of this program, all sensors have the same time interval of 2s;
  //to be reworked for the final version
